...
const vendorSource = path.join(root, 'src/vendor/credential-reporter.js');
...
check('probes role-scoped routes, never /health', () => {
  const text = fs.readFileSync(path.join(root, 'src/credential-self-reporter.ts'), 'utf8');
  assert(
    text.includes('/api/orders/internal/order-affinity/replay-candidates'),
    'does not probe the order-affinity replay route',
  );
...
check('principal matches auth exactly', () => {
  const text = fs.readFileSync(path.join(root, 'src/credential-self-reporter.ts'), 'utf8');
  for (const p of [
    'svc-marketing-microservice--orders-microservice@internal.alfares.cz',
  ]) {
...
check('postbuild copies the vendored module into dist', () => {
  // This repo builds with plain `tsc`, which has NO asset copier: tsconfig
  // includes only src/**/* and allowJs is off, so a vendored .js is silently
  // dropped from dist/. Without an explicit copy the pod throws
  // MODULE_NOT_FOUND at boot.
  const pkg = JSON.parse(fs.readFileSync(path.join(root, 'package.json'), 'utf8'));
  const postbuild = (pkg.scripts || {}).postbuild || '';
  assert(
    postbuild.includes('vendor'),
    'package.json has no postbuild step copying src/vendor into dist/',
  );
});
...
check('vendored module reached dist/ (run after build)', () => {
  ...
  const candidates = fs
    ...
    .map((e) => path.join(root, 'dist', e.name, 'credential-self-reporter.js'))
    .concat([path.join(root, 'dist/credential-self-reporter.js')]);
