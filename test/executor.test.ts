...
test("audit logger forwards sanitized payload to logging service when configured", async () => {
  ...
  try {
    ...
    assert.equal(calls[0].payload.service, "marketing-microservice");
    assert.equal(calls[0].payload.duration_ms, 7);
    assert.equal((calls[0].payload.metadata as Record<string, unknown>).event, "audit_forward_test");
    assert.equal((calls[0].payload.metadata as Record<string, unknown>).secret, "[redacted]");
