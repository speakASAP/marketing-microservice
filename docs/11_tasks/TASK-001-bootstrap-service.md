# Task: Marketing Microservice IPS adoption bootstrap
...
This task does not change MKT-INV-001..006; it documents them formally in docs/17_governance/PROJECT_INVARIANTS.md for the first time.

## sensitive-data classification

No secrets, campaign recipient PII, or production data are included in any adoption artifact; all content is architectural/process documentation drawn from non-secret repository files.
...
This task does not change the Orders-events consumer or any campaign execution determinism behavior; it only documents existing facts.
...
- Resolving the open attribution-ownership or CRM/account-integration follow-ups
- Modifying docs/orchestrator/ROADMAP.md or docs/agents/* contract content

## acceptance criteria

- The IPS planning validator passes with no unresolved findings for marketing-microservice
...
- src/orders-events-consumer.ts for confirming RabbitMQ usage
...
This is a single-owner documentation bootstrap with no parallel workstreams; it does not touch campaign execution code or the open attribution/CRM follow-ups.
